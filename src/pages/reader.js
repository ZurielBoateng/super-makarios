...
export async function renderReader(root, bookId, { onExit } = {}) {
  ...
  let book, rendition;
  let bookRow;
  let fontStepIndex = FONT_STEPS.indexOf((await getSetting('reader:fontStep')) ?? 100);
  ...
  try {
    ...
    rendition.on('rendered', () => {
      try {
        ...
        if (!doc) return;
        let startX = 0;
        doc.addEventListener('touchstart', (e) => (startX = e.changedTouches[0].screenX));
  ...
  els.bookmarkBtn.addEventListener('click', async () => {
    ...
    if (marked) {
      ...
      await addBookmark(bookId, { cfi: currentCfi, label: `${percent}% through` });
      toast('Bookmarked');
    }
    await refreshBookmarkIcon();
  ...
  function paintDownloadIcon() {
    ...
    els.downloadBtn.title = downloaded ? 'Downloaded for offline — tap to remove' : 'Download for offline';
  }
  ...
  function openSettingsPanel() {
    ...
    document.getElementById('show-bookmarks').addEventListener('click', openBookmarksPanel);
  }

  async function stepFont(dir) {
    fontStepIndex = Math.min(FONT_STEPS.length - 1, Math.max(0, fontStepIndex + dir));
    rendition.themes.fontSize(`${FONT_STEPS[fontStepIndex]}%`);
  ...
  async function openBookmarksPanel() {
    ...
    document.getElementById('show-settings').addEventListener('click', openSettingsPanel);
  }
